...
function aiAnswer(q){
  ...
  if(/about|company|business|established|owner|chairman|ceo|leadership/.test(s)) return 'PR Rental Services was established in 2025 and operates as a service provider and proprietorship. Praveen Polepalli is Chairman and Proprietor. Pratibha Polepalli is part of the PR Rental Services leadership team.';
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){productModal.classList.remove('open');qrModal.classList.remove('open');ai.classList.remove('open');document.getElementById('nav').classList.remove('open');}});
